...
def rf_generic_all_navarra(path_to_data, split_number):
    """
    ...
    df1 = pd.read_table(path_to_data, sep=',', error_bad_lines=False)
    df = df1.replace('nan', float(-9999), regex=True)
    feature_cols = get_feature_cols(df)
    features_dataframe = df[feature_cols]
    classes_dataframe = df.loc[:, ['IDClaseGen']]

    for index, row in classes_dataframe.iterrows():
        print('Este es el indice: ')
        print(index)
        print(classes_dataframe.iloc[index])
        print("Tipo:  ")
        print(type(classes_dataframe.iloc[index]))
    ...
    for train_index, test_index in index_iterator:
        ...
        X_train[X_train == ' '] = float(-9999)
        X_test[X_test == ' '] = float(-9999)
        ...
        for i in x_train_null_values:
            X_train[i[0], i[1]] = float(-9999)
        for i in x_test_null_values:
            X_test[i[0], i[1]] = float(-9999)
...
rf_generic_all_navarra('/home/naroairirarte/Desktop/prueba_rf/t1.txt', 2)